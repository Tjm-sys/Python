...
def mover_bloco_sgiro(potencia, num_graus_distancia):
    ...
    while abs(motor_esquerda.angle()) < num_graus_distancia:   
        ...
        correcao_bloco_mov = diferenca_bloco_mov * 2   # Ajustar o valor "2" se necessario
...
while True:
    if proximidade_sensor.distance() > 150:  # Distância em mm
        virar_bloco_sgiro(90, "direita", 50) # angulos par, e potencia impar
        mover_bloco_sgiro(50, 90)
        virar_bloco_sgiro(90, "esquerda", 50)
        mover_bloco_sgiro(50, 90)
        virar_bloco_sgiro(90, "esquerda", 50)
        mover_bloco_sgiro(50, 90)
        virar_bloco_sgiro(90, "direita", 50)
    else:
        if sensor_direita.color() == "Green" and sensor_esquerda.color() != "Green":
            # girar para a direita
            print("teste = verda na direita")
        elif sensor_esquerda.color() == "Green" and sensor_direita.color() != "Green":
            # girar para a esquerda
            print("teste = verde na esquerda")
        elif sensor_direita.color() == "Green" and sensor_esquerda.color() == "Green":
            # girar 180°
            ...
            correcao = 6 * ref_sensores_diferenca
    ...
    # Chamar função mostrar a cada 1000 repetições
        contador = contador + 1
        if contador >= 1000:
            mostrar()
            contador = 0
